...
export async function POST(req: Request) {
  try {
    ...
    if (state && state.resetAt > now && state.count >= 8) return NextResponse.json({ error: "Too many attempts. Try again later." }, { status: 429 });
    ...
    return NextResponse.json({ error: e?.issues?.[0]?.message || "Invalid login data." }, { status: 400 });
  }
}
